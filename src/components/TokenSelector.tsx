...
const TokenSelector: React.FC<TokenSelectorProps> = ({
  ...
  const filteredTokens = tokens.filter(token =>
    ...
    const popularTokens = ['SOL', 'USDC', 'USDT', 'RAY', 'SRM', 'ORCA', 'MNGO', 'BONK', 'WIF', 'POPCAT']
  ...
  return (
    <div className="relative">
      <button
        ...
        className="btn btn-secondary flex items-center gap-3 min-w-[140px]"
      >
        {loading ? (
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 glass p-4 z-10 max-h-80 overflow-hidden">
            ...
            {filteredTokens.slice(0, 30).map((token) => (
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <div style={{ color: 'white', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.95rem' }}>
                    {token.symbol}
                    {['SOL', 'USDC', 'USDT', 'RAY', 'SRM', 'ORCA', 'MNGO', 'BONK', 'WIF', 'POPCAT'].includes(token.symbol) && (
                  ...
                  <div style={{ color: '#9ca3af', fontSize: '0.8rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{token.name}</div>
                </div>
              </button>
            ))}
